// Brut Force
var majorityElementWithBruteForce = function (nums) {
  const n = nums.length;
  for (let i = 0; i < n; i++) {
    let count = 0;
    for (let j = 0; j < n; j++) {
...
// Sorting
var majorityElementWithSorting = function (nums) {
  nums.sort((a, b) => a - b);
  return nums[Math.floor(nums.length / 2)];
};

// Using HashMap
var majorityElementWithHashmap = function (nums) {
  const counts = {};
...
var majorityElementWithMooreAlgo = function (nums) {
